...
describe('Starting tests for fintech3', function() {
  ...
  it('website title should be fintech3', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('fintech3');
    })
  });
  ...
  it('navbar-brand should be fintech3',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('fintech3');
    });
  });
